...
def encode_deeplink_payload(anilist_id: int, ep_number: str, quality: str = "720p") -> str:
    """Encodes anilist_id, episode number, and optional quality into a URL-safe Base64 payload string."""
    raw_str = f"{anilist_id}:{ep_number}:{quality}"
...
def decode_deeplink_payload(payload: str) -> Optional[Dict[str, Any]]:
    """Decodes a start payload string into anilist_id, ep_number, and quality."""
    if not payload:
        return None
    try:
